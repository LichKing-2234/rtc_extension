...
enum VideoPixelFormat {
  ...
  PIXEL_FORMAT_MJPEG =
      14, // MJPEG compressed.
          // MediaTek proprietary format. MT21 is similar to NV21 except the
          // memory layout and pixel layout (swizzles). 12bpp with Y plane
          // followed by a 2x2 interleaved VU plane. Each image contains two
          // buffers -- Y plane and VU plane. Two planes can be non-contiguous
          // in memory. The starting addresses of Y plane and VU plane are 4KB
          // alignment. Suppose image dimension is (width, height). For both Y
          // plane and VU plane: Row pitch = ((width+15)/16) * 16. Plane size =
          // Row pitch * (((height+31)/32)*32)
...
public:
  ...
  virtual int SetupProvider(const char *provider_name, int provider_name_length,
  ...
  virtual int OnIncomingCapturedI420Data(
      ...
      VideoPixelFormat pixel_format, int frame_feedback_id = 0) = 0;

  virtual void OnError(const char *provider_name,
                       const char *provider_name_length, const char *reason,
                       int reason_length) = 0;
};
